...
import { 
  ...
  GardenSettings, StreakData, ESPECIO_DATABASE, BiomeType, Anotacao,
...
// Lucide icons
import { 
  BarChart2, BookOpen, Clock, Layers, HelpCircle, 
  Plus, Trash2, Calendar, FileText, CheckSquare, 
  Settings, Undo2, LogOut, Sun, Moon, Sparkles, Droplets, Info,
  CheckCircle, AlertCircle, AlertTriangle, X,
  LayoutDashboard, Sprout, Timer, GraduationCap, History,
  Flame, Award, Trophy, BookMarked, Star, Volume2, VolumeX, Lightbulb, PenTool,
  Search, Filter
} from 'lucide-react';
...
export default function App() {
  ...
  const showToast = (message: string, type: 'success' | 'error' | 'info' | 'warning' = 'info') => {
    ...
    // Play pleasant success sound on achievements
    if (type === 'success') {
  ...
  // Custom styled confirmation modal state (replaces native thread-blocking confirm())
  ...
  const [gardenSettings, setGardenSettings] = useState<GardenSettings>({
    ...
    fertilizerReserve: 2,
    unlockedLandmarks: [],
    focusPoints: 30, // starter focus points
    survivalMode: false,
  ...
  // Safe sync ref update — no external side effect
  ...
  function generateUniqueStringId(): string {
  ...
  useEffect(() => {
    try {
      ...
      if (gs && parsedSettings) {
        if (parsedSettings.autoBackupEnabled === undefined) {
          parsedSettings.autoBackupEnabled = true;
        }
        setGardenSettings(parsedSettings);
      }

      // Apply dynamic plant survival withering check if enabled
  ...
  useEffect(() => {
    if (!isLoadedRef.current) return;
    localStorage.setItem('sessoesEstudo', JSON.stringify(sessoesEstudo));
  }, [sessoesEstudo]);
  ...
  useEffect(() => {
    if (!isLoadedRef.current) return;
    localStorage.setItem('gardenSettings', JSON.stringify(gardenSettings));
  }, [gardenSettings]);
  ...
  useEffect(() => {
    if (!isLoadedRef.current) return;
    localStorage.setItem('estufaFotos', JSON.stringify(fotos));
  }, [fotos]);

  useEffect(() => {
    if (!isLoadedRef.current) return;
    if (gardenEvent) {
      localStorage.setItem('gardenEvent', JSON.stringify(gardenEvent));
    } else {
      localStorage.removeItem('gardenEvent');
    }
  }, [gardenEvent]);

  // Handle Automatic Backup in LocalStorage on significant actions (with 1 second debounce)
  useEffect(() => {
    if (!isLoadedRef.current) return;
  ...
  // Trigger Biome achievement automatically when extra biomes are unlocked
  useEffect(() => {
  ...
  // Reset reviewed session automatically when a new day starts
  useEffect(() => {
    setReviewedThisSession([]);
  }, [getTodayDate()]);

  // Keep selectedTreeForNurture synchronized with plantedTrees
  useEffect(() => {
    if (selectedTreeForNurture) {
      const currentTree = plantedTrees.find(t => t.id === selectedTreeForNurture.id);
      if (currentTree) {
        if (currentTree !== selectedTreeForNurture) {
          setSelectedTreeForNurture(currentTree);
        }
      } else {
  ...
  // Helpers for achievements and timeline log
  ...
  const triggerUnlockAchievement = (id: string) => {
    ...
    if (ach && !ach.desbloqueado) {
      ...
      setGardenSettings(prevSettings => ({
        ...prevSettings,
        focusPoints: prevSettings.focusPoints + ach.xpBonus
      }));
  ...
  // --- ACTIONS HISTORIES FOR UNDO ---
  const addUndoAction = (desc: string, undoFn: () => void) => {
    setHistoryStack(prev => {
      const updated = [...prev, { desc, undoFn }];
      if (updated.length > 15) updated.shift(); // keep last 15 acts
      return updated;
    });
  };
  ...
  const registerProgress = (tipo: 'questoes' | 'revisoes' | 'leituras' | 'tempoMin', value = 1) => {
    ...
    setStreak(prev => {
      ...
      if (lastComp) {
        ...
        if (lastCompObj < yesterday) {
          currStreak = 0; // streak lost!
        }
      }

      // Check if all goals are fully achieved today using metasRef.current
      ...
      if (fullyAchieved && lastComp !== hoje) {
        lastComp = hoje;
        currStreak += 1; // build study streak count!
      }
  ...
  const handleAddConteudo = () => {
    ...
    // Spaced repetition scheduler! (1, 7, 30, 60 days standard)
    ...
    setMaterias(prev => {
      ...
      return updatedMaterias.map(m => {
        if (m.id === targetMateriaId) {
          return {
            ...m,
            conteudos: [...m.conteudos, newCont]
          };
        }
        return m;
      });
    ...
    // Clear con filters so user sees newly added content immediately
    setConSearchQuery('');
      ...
      onConfirm: () => {
        ...
        setMaterias(prev => prev.map(m => {
          if (m.id === materiaId) {
            return {
              ...m,
              conteudos: m.conteudos.filter(c => c.id !== conteudoId)
            };
          }
          return m;
        }).filter(m => m.nome === 'Geral' || m.conteudos.length > 0)); // remove dry subjects

        setRevisoes(prev => prev.filter(r => r.grupoId !== grupoId));

        addUndoAction(`Remover assunto`, () => {
          setMaterias(oldMaterias);
          setRevisoes(oldRevs);
        });
      }
    });
  ...
  const handleSaveNotes = (materiaId: number, conteudoId: string, notes: string) => {
    setMaterias(prev => prev.map(m => {
      if (m.id === materiaId) {
        return {
          ...m,
          conteudos: m.conteudos.map(c => c.id === conteudoId ? { ...c, anotacoes: notes } : c)
        };
      }
      return m;
    }));
  ...
  const handleMarkRevisaoCompleted = (id: number, options = { addUndo: true }) => {
    ...
    if (rev.ciclo) {
      // Completed, remove from future queues
      ...
      setGardenSettings(prev => ({
        ...
        focusPoints: prev.focusPoints + 15
      }));
    } else {
      // Manual spaced review, increment level
      const newNivel = Math.min(rev.nivel + 1, 4);
      const nextDate = addDays(getTodayDate(), newNivel === 2 ? 3 : newNivel === 3 ? 7 : 30);
      
      setRevisoes(prev => prev.map(r => r.id === id ? {
        ...r,
        nivel: newNivel,
        ultima: getTodayDate(),
        proxima: nextDate,
  ...
  // --- STUDY NOTES (ANOTAÇÕES & INSIGHTS) ---
  const handleSaveNota = () => {
    ...
    if (nNotaAtivaId) {
      ...
      setAnotacoes(prev => prev.map(n => n.id === nNotaAtivaId ? {
        ...
        // recalculate possible rewards if not claimed yet
      ...
      if (charCount >= 200) {
        triggerUnlockAchievement('2'); // "Escritor Ativo"
      }
  ...
  const handleGenerateAIFlashcards = async () => {
    ...
    if (!nConteudo.trim()) {
      return showToast('Por favor, escreva alguma anotação antes de gerar Flashcards com IA!', 'warning');
    }
    ...
    try {
      ...
      if (data.flashcards && Array.isArray(data.flashcards) && data.flashcards.length > 0) {
        ...
        if (dedupedGenerated.length > 0) {
          ...
          addUndoAction(`Desfazer geração de ${dedupedGenerated.length} flashcards`, () => {
            setFlashcards(prev => prev.filter(f => !dedupedGenerated.some(g => g.id === f.id)));
          });
  ...
  const handleClaimNotaRecompensas = (id: number) => {
    ...
    if (nota.caracteres < 50) {
      return showToast('Insira mais profundidade na sua anotação (mais de 50 caracteres) para desbloquear seu bônus intelectual!', 'warning');
    }
    ...
    addUndoAction('Desfazer resgate de recompensa', () => {
      setGardenSettings(oldGardenSettings);
      setAnotacoes(oldAnotacoes);
    });
  ...
  const handleAddQuestoes = (assunto: string, total: number, acertos: number) => {
    ...
    // Give some focus points depending on the correct answers score!
    const earnedPoints = total > 0 ? Math.round((acertos / total) * 30) : 0;
    setGardenSettings(prev => ({
      ...prev,
      focusPoints: prev.focusPoints + earnedPoints,
      waterReserve: prev.waterReserve + (total >= 10 ? 1 : 0) // award water bottle for resolving > 10 questions
    }));

    // Trigger achievements & timeline logs
    triggerUnlockAchievement('3'); // "Mestre Examinador"
  ...
  // --- COMBINED ACTIVE STUDY TIMER COMPLETE WORKFLOW ---
  const handleActiveSessionComplete = (assuntoName: string, durationMin: number, chosenSpeciesId?: string) => {
    ...
    // 2. Automated spaced recurrence resolving!
    // If the subject studied matches a pending spaced repetition today, mark it completed automatically!
    ...
    if (matchedPendingRev) {
      handleMarkRevisaoCompleted(matchedPendingRev.id, { addUndo: false });
    }

    // 3. Determine tree species from SPECIES catalog database
    let speId = 'bonsai_sabedoria'; // default common tree
    ...
    if (chosenSpeciesId) {
      ...
      if (durationMin >= 90) {
        ...
        // Dynamic unlocks based on keywords of subject Name
      ...
      // Unlocks because of streak data
      const currentStreakVal = (() => {
        ...
        todayCounts.tempoMin += durationMin; // Add the current session duration
        ...
        if (fullyAchieved && streakRef.current.lastCompletedDate !== hoje) {
          return currentStreakBase + 1;
        }
    ...
    if (hasPlanted) {
      // 4. Position spawning logic (avoid overlay clusters)
      // Coordinates placed inside a 15% to 85% safe-padding field box
      ...
      const newTree: Tree = {
        ...
        waterDroplets: 0,
        fertilizerAmount: 0,
        // DIFICULTANDO: sementes começam com apenas 0% ou pouco progresso, exigindo dedicação real
        growthPercent: durationMin >= 50 ? 15 : (durationMin >= 30 ? 10 : 0),
        phase: 'seed', // Sempre começa como semente independente do tempo, para valorizar a evolução!
        positionX: randX,
        positionY: randY
      };
      ...
      setGardenSettings(prev => ({
        ...prev,
        focusPoints: prev.focusPoints + xpBonus,
        waterReserve: prev.waterReserve + earnedWater,
        fertilizerReserve: prev.fertilizerReserve + earnedFertilizer
      }));

      // Log in timeline and check achievement
      ...
      triggerUnlockAchievement('1'); // "Primeiro Foco"

      showToast(`🌳 ÁRVORE PLANTADA! ${descReward} (+${xpBonus} XP, +${earnedWater} 💧)`, 'success');
      setActiveTab('plantacao'); // switch to plantation tab automatically to let them water the seed!
    } else {
      setGardenSettings(prev => ({
        ...prev,
        focusPoints: prev.focusPoints + xpBonus,
  ...
  const handleCreateFlashcard = () => {
    ...
    addUndoAction('Criar flashcard', () => {
      setFlashcards(oldCards);
    });
  ...
  const handleTriggerAutomatedGeneration = async (subjectName: string) => {
    ...
    try {
      // Find relevant study notes in our local state to personalize the cards
      const matchingNotes = anotacoesRef.current.filter(
        note => note.assunto?.toLowerCase().trim() === subjectName.toLowerCase().trim()
      );
      const notesContext = matchingNotes
        .map(note => `[Anotação: ${note.titulo}]\n${note.conteudo}`)
        .join("\n\n");
      ...
      const response = await fetch(`${API_BASE}/api/gemini/generate-flashcards-by-subject`, {
        ...
        body: JSON.stringify({
          assunto: subjectName,
          notasRelacionadas: notesContext || undefined
        })
      });

      if (!response.ok) {
        throw new Error('Falha de resposta ao conectar com a Estufo do Gemini.');
      }
      ...
      if (data.flashcards && Array.isArray(data.flashcards) && data.flashcards.length > 0) {
        ...
        if (deduped.length > 0) {
          ...
          addUndoAction(`Desfazer geração de ${deduped.length} flashcards`, () => {
            setFlashcards(prev => prev.filter(f => !deduped.some(g => g.id === f.id)));
          });
  ...
  const handleExplainFlashcard = async (fc: Flashcard) => {
    ...
    try {
      ...
      if (data.explanation) {
        ...
        showToast('Não foi possível obter uma explicação detalhada no momento.', 'warning');
      }
    } catch (err: any) {
      console.error(err);
  ...
  const handleFlashcardRecall = (id: number, correct: boolean) => {
    ...
    setFlashcards(prev => prev.map(fc => {
      if (fc.id === id) {
        ...
        if (correct) {
          newSeq += 1;
          newInterval = Math.round(fc.intervaloDias * 2); // Double spaced repetition days
          if (newInterval > 90) newInterval = 90;
        } else {
          newSeq = 0;
          newInterval = 1; // Back to daily reviews
        }
        
        const nextDt = addDays(getTodayDate(), newInterval);
        
        return {
          ...fc,
          sequenciaAcertos: newSeq,
          intervaloDias: newInterval,
          proximaData: nextDt
        };
      }
      return fc;
    ...
    if (correct) {
      // Award reward focus points and droplets!
      setGardenSettings(prev => ({
        ...prev,
        focusPoints: prev.focusPoints + 15, // +15 FP
        waterReserve: prev.waterReserve + (Math.random() > 0.6 ? 1 : 0) // Chance of water droplets!
      }));
      triggerUnlockAchievement('5'); // "Sábio Memorizador"
    ...
    // Close card study view or select next Card
    setFcActiveId(null);
    setFcFlipped(false);
  };

  // --- NOTES EXPORTERS (MARKDOWN & PDF) ---
  ...
  const handlePrintPDF = (nota: Anotacao) => {
    ...
    if (!printWindow) {
      showToast('Por favor, permita que pop-ups apareçam para exportar PDF.', 'warning');
      return;
    }
    ...
    const escapedTitulo = escapeHtml(nota.titulo);
    const escapedAssunto = escapeHtml(nota.assunto);
    const escapedConteudo = escapeHtml(nota.conteudo);
    ...
    const htmlContent = `
      <html>
        ...
        <body>
          <h1>${escapedTitulo}</h1>
          <div class="meta">Tópico: ${escapedAssunto} | Registrado em: ${nota.data} | Comprimento: ${nota.caracteres} caracteres</div>
          <div class="content">${escapedConteudo}</div>
  ...
  const handleImportBackup = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        if (data.app !== "CultivaMente" || !data.payload) {
          throw new Error("Formato de backup inválido. Certifique-se de que é um arquivo gerado pelo CultivaMente.");
        }

        const payload = data.payload;

        // Confirm restoration first
        setConfirmModal({
  ...
  const handleAutoBackup = () => {
    ...
    try {
      ...
      localStorage.setItem('cultivamente_auto_backup', fileData);
    } catch (e) {
  ...
  const handleRestoreAutoBackup = () => {
    try {
      ...
      if (!raw) {
        showToast("Nenhum backup automático encontrado no LocalStorage.", "warning");
        return;
      }
  ...
  const handleAddLeitura = (
    ...
    if (tipo === 'Artigo') {
      ...
      // Base de 30 XP + 1.5 XP por página de artigo científico
      ...
      // Base de 100 XP + 0.5 XP por página para livros inteiros
      ...
      // Filme com XP atrás de livro e artigos por unidade, mas escalável pelo tempo + bônus
      ...
      // Podcast com menos valor unitário que artigos, dependendo do tempo rende mais XP
      ...
      // Artigo científico escrito: +180 XP base e alta recompensa
      ...
      // TCC/Monografia é o maior booster intelectual autoral: +300 XP base
      ...
      // Resumos têm ganho balanceado por compressão de conteúdo
      ...
      // Redação/Ensaios: foco em criatividade e coesão textual
      ...
      // Investigações e crônicas autorais
      ...
      // Micro-ensaios / threads educativas
    ...
    // Play immersive procedural sound
    if (tipo === 'Livro') {
      audioSynth.playReadBook();
    } else if (tipo === 'Artigo') {
      audioSynth.playReadArticle();
    } else if (tipo === 'Filme') {
    ...
    // Award custom gamified currencies
    setGardenSettings(prev => ({
      ...prev,
      focusPoints: prev.focusPoints + earnedXP,
      waterReserve: prev.waterReserve + earnedWater,
      fertilizerReserve: prev.fertilizerReserve + earnedFertilizer
    }));

    // Trigger achievements & timeline events
    triggerUnlockAchievement('6'); // "Leitor Voraz"
    addTimelineEvent('Consumo de Conhecimento 📚', `Registrou leitura (${tipo}): "${titulo}" sobre "${assunto || 'Geral'}"!`, 'estudo', '📚');

    addUndoAction(`Adicionar leitura de ${tipo}`, () => {
      setLeituras(oldLeituras);
      setGardenSettings(oldGardenSettings);
    });
  ...
  const getSubjectSuggestions = () => {
    ...
    materias.forEach(m => {
      list.add(m.nome);
      m.conteudos.forEach(c => list.add(c.nome));
    });
    return Array.from(list).filter(Boolean);
  };

  // --- FILTERED MATERIAS FOR CONTEUDOS TAB ---
  ...
  const filteredMaterias = useMemo(() => {
    return materias.map(m => {
      if (conFilterMateriaId !== 'all' && m.id !== conFilterMateriaId) {
        return null;
      }
  ...
  const totalConteudosFiltrados = useMemo(() => {
    return filteredMaterias.reduce((acc, m) => acc + m.conteudos.length, 0);
  }, [filteredMaterias]);
